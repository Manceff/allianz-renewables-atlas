...
Tes agents Claude Code attaquent en 4 phases (le scaffold du Globe / Spotlight / About est déjà en place — les agents implémentent la donnée + les libs internes) :

1. **Phase 1 — Cartographie ACP** (Jour 1-1.5) : enrichir `data/parks_index.yaml` de 10 → 23 parcs en scrapant les press releases ACP. Output : YAML validé Pydantic + UNCERTAIN.md pour les ambigus.

2. **Phase 2 — Libs internes TDD** (Jour 1.5-3) : implémenter `src/lib/parks_loader.py` (YAML→Pydantic), `src/lib/pvgis_fetch.py` (HTTP JRC), `src/lib/sentinel_fetch.py` (OAuth Copernicus), `src/lib/compute_delta.py`. Tests pytest avant chaque lib.

3. **Phase 3 — Precompute exhaustif** (Jour 3) : exécuter `scripts/precompute_all.py` sur les 23 parcs. Génère `data/parks/<id>/{metadata.json, sentinel.png, production_estimated.json, delta.json}`. Failures → `PRECOMPUTE_FAILURES.md`.

4. **Phase 4 — Polish UI + deploy** (Jour 3-4) : finaliser logique des 3 pages (les stubs UI sont déjà fonctionnels), GitHub Actions cron hebdo, deploy Streamlit Cloud, mail analyste.
